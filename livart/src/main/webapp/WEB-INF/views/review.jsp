...
<script>
    !function(f,b,e,v,n,t,s){
      ...
      fbq('init', '865758067551816');
      fbq('init', '555403818712228'); // 차이
      fbq('track', 'PageView');
...
<script type="text/javascript"> 
    ...
    if (!_nasa) var _nasa={};
    if(window.wcs){
      wcs.inflow("hyundailivart.co.kr");
      wcs_do(_nasa);
    }
  </script>
<!-- E : 차이 - 네이버 공통 적용 스크립트 -->
...
<script>
    ...
    while ( x <= document.cookie.length ) {
      ...
      if (document.cookie.substring( x, y ) == cookieName) {
        ...
        gaCid = decodeURI(document.cookie.substring(y, lastChrCookie)).replace("GA1.3.","");
       }
    ...
    console.log("gaUid > " + gaUid);
    console.log("gaHid > " + gaHid);
    
    gtag('set', 'dimension1', gaCid);
    
    if('N' === 'N'){ // 비로그인 시
      gtag('config', 'UA-91366621-1');
    
    } else if(gaHid == null || gaHid == "") { // 통멤회원 X
      gtag('set', 'dimension2', gaUid);
      gtag('config', 'UA-91366621-1');
      
    } else {		// 통멤회원 X
      gtag('set', 'dimension2', gaUid);
      gtag('set', 'dimension3', gaHid);
      gtag('config', 'UA-91366621-1');
    }
    
    gtag('config', 'AW-388435060');		// [플레이디] Google Ads
...
<script type="text/javascript">
  $(document).ready(function () {
    ...
    // QuickMenu
    $('#quickmenu1, .quick-content-close__btn').on('click', function () {
    ...
    $("#button-quick-community").hide();
    $('#button-quick-mall').show();
    $('.quick').css({ left: '0', right: 'inherit' });
    anime({
      targets: '.page-wrapper',
      translateX: '-100%',
      duration: 800,
      easing: 'cubicBezier(0.17, 0.04, 0.03, 0.94)',
    });
    $('#pageMall').css({ left: '-100vw', position: 'absolute' });
    $('#pageCommunity').css({ left: '0', position: 'relative' });
  ...
  $(window).on('scroll', function () {
    if ($('#tabDetail').length !== 0) {
      if ($('#tabDetail').offset().top <= $(window).scrollTop()) {
        $('#tabDetail .tab-detail').css({
          position: 'fixed',
        });
      } else {
        $('#tabDetail .tab-detail').css({
  ...
  // 200821, fullmenu swiper
  var swiper = new Swiper('.fullmenu-swiper', {
    navigation: {
  ...
  // S : 리바트몰-커뮤니티 페이지 전환 : 200903
  var swiperQuickRecent;
  function swiperLoadQuickRecent() {
    // 최근 본 상품 스와이프
    var swiperList = $('.quick-item-recent-list .swiper-slide'),
      swiperControl = $('.quick-item-recent-list .swiper-controls-wrap');
  
    if (swiperList.length > 4) {
      swiperQuickRecent = new Swiper('.quick-item-recent-list .swiper-container', {
        loop: false,
        direction: 'vertical',
        navigation: {
          nextEl: '.quick-item-recent-list .swiper-button-next',
          prevEl: '.quick-item-recent-list .swiper-button-prev',
        },
  ...
  // 퀵 메뉴의 최근본상품~하단까지 높이 체크
  function checkQuickRecentHeight() {
    // minHeight : 퀵 메뉴의 최근본상품~하단까지 높이
    var minHeight = 270;
    var quickContentHeight = $('.quick-content').outerHeight();
    var topHeight = minHeight - quickContentHeight;
    if (topHeight < 0) {
      topHeight = topHeight + 'px';
      $('.quick-item-recent-list').css({ top: topHeight });
    } else {
      $('.quick-item-recent-list').css({ top: 0 });
    }
  }
  
  $(document)
    //	quick 이벤트 수정 : 201116
    // .on('mouseenter', '.button-switch-quick', function () {
    .on('mouseenter', '.quick-inner', function () {
      $(this).closest('.quick').addClass('__quick-open');
    })
    .on('mouseleave', '.quick', function () {
      $(this).closest('.quick').removeClass('__quick-open');
    })
    .on('click', '#button-quick-community', function () {
      $(location).attr("href","https://www.hyundailivart.co.kr/community");
    })
    .on('click', '#button-quick-mall', function () {
      $(location).attr("href","https://www.hyundailivart.co.kr/index");
    })
    .on('click', '.quick-menu-item', function (e) {
      if ($(this).attr('id') == 'button-quick-menu-recent') {
        // Quick 최근 본 상품 클릭시
        e.preventDefault();
  
        if ($(this).hasClass('is-active')) {
          $(this).removeClass('is-active');
          $('.quick-item-recent-list').hide();
          if (swiperQuickRecent != undefined) swiperQuickRecent.destroy();
        } else {
          $(this).addClass('is-active');
          $('.quick-item-recent-list').show();
          // 최근 본 상품 스와이프
          swiperLoadQuickRecent();
  
          // 퀵 메뉴의 최근본상품~하단까지 높이 체크
          checkQuickRecentHeight();
        }
      } else {
        $('#button-quick-menu-recent').removeClass('is-active');
        $('.quick-item-recent-list').hide();
        if (swiperQuickRecent != undefined) swiperQuickRecent.destroy();
      }
    })
    .on('click', '.quick-item-recent-list .btn-del', function () {
      // Quick 최근 본 상품 목록 삭제
      var tg = $(this).closest('.swiper-slide').index();
      $('.quick-item-recent-list .swiper-slide').eq(tg).remove();
      var tgLen = $('.quick-item-recent-list .swiper-slide').length;
      if (tgLen < 1) {
        $('.quick-content').hide();
        $('#button-quick-menu-recent').removeClass('is-active').addClass('is-disabled');
      }
      $('#button-quick-menu-recent .quick-menu__num').text(tgLen);
      // swiper.updateSlides();
      swiperLoadQuickRecent();
  
      // 퀵 메뉴의 최근본상품~하단까지 높이 체크
      checkQuickRecentHeight();
      data = "sn=" +  $(this).data("val");
      deleteGoodsHistory(data);
    });
  
    var scrollPosition;
    function fn_scroll_updown(scr) {
      var retStr;
      if (scr > scrollPosition) {
        // scrollDown Check
        retStr = 'down';
      } else {
        // scrollUp	Check
    ...
    function fn_swichbutton_scroll() {
      var winHeight = $(window).innerHeight();
      var quickHeight = $('.quick-inner').height();
      var scroll_top = $(window).scrollTop();
      var topPx = winHeight - quickHeight;
      topPx = topPx + 'px';
      if (winHeight > quickHeight) {
        $('.quick-inner').css({ top: 0 });
      } else {
        if (fn_scroll_updown(scroll_top) == 'down') {
          $('.quick-inner').css({ 'margin-top': topPx });
        } else {
          $('.quick-inner').css({ 'margin-top': 0 });
        }
      }
    }
    ...
    function deleteGoodsHistory(data) {
  
      $.ajax({
            url : "/goods/deleteHistory",
            dataType : "json",
            data : data,
    ...
    function heart(obj, blbsn){
        /*좋아요 카운트 증가,해제*/
  
        var $that = $(this);
        var $icoHeart = $(obj).find('.product-item-heart');
        var num = parseInt(removeComma($(obj).find('.product-item-heart__num').text()));
        $.ajax({
              url: "/community/mylove",
              data: {blbSn : blbsn},
              dataType:"json",
              success: function (result) {
                  if(result.resultData === "0000") {
                    if(!$icoHeart.hasClass('is-active')){
                      $icoHeart.addClass('is-active');
                      num = num + 1;
                    }else{
                      $icoHeart.removeClass('is-active');
                      num = num <= 0 ? 0 : num - 1;
                    }
                    var txtNum = num.toLocaleString();
                    $(obj).find('.product-item-heart__num').text(txtNum);
  
                  }else{
                      $.eshopfront.cmm.alert({
                          content : "좋아요는 한번만 가능합니다.",
                          btn: {
                                  close: false,
                                  cancel: false
                              }
      ...
      function popupCpnDownload(cpnSn, popSn){
      //쿠폰다운로드
      if('N' === 'N'){
          $.eshopfront.cmm.doLogin();
        return false;
      }else{
          var data = {"cpnSns" : cpnSn}
          $.eshopfront.cmm.issueCoupon(data, function(result){
          if(result.resultCode == "00"){
            var cookieNm = 'beforeUrlPopUp_'+popSn;
            $.eshopfront.cmm.removeCookie(cookieNm,".hyundailivart.co.kr");
            $.eshopfront.cmm.setCookie(cookieNm,"N",1,".hyundailivart.co.kr");
            $.eshopfront.cmm.alert({
              content: "<p>쿠폰을 다운로드 받으셨습니다.</p>",
              btn: {
                close: false,
                cancel: false,
              },
              callback: {
                close: function(){
                  $(".button-modal-close", ".modal[id*='popup_']").click();
                  $(".modal[id*='popup_']").removeClass("is-active");
...
<%@ include file="../../static/header.jsp" %>
	...
	<script type="text/javascript">
        $(document).ready(function(){
          ...
          $(document)
            .on('click', '.button-comm-write', function () {
              $('#modal-comm-selectcontent').show();
              $(this).addClass('is-active').html('<i class="ico-cross is-size-4"></i>닫기');
            })
            .on('click', '.button-comm-write.is-active', function () {
              $('#modal-comm-selectcontent').hide();
              $(this).removeClass('is-active').html('<i class="ico-comm_content"></i>콘텐츠 등록');
            });
          
          // GNB 카테고리 메뉴 클릭
          /* $('.fullmenu__btn').on('click', function () {
            if ($(this).hasClass('is-active')) {
              $(this).removeClass('is-active');
              $(this).closest('.gnb-category-wrap').find('.fullmenu-wrap').removeClass('open');
              $(this).children('span').text('풀메뉴 열기');
            } else {
              $(this).addClass('is-active');
              $(this).closest('.gnb-category-wrap').find('.fullmenu-wrap').addClass('open');
              $(this).children('span').text('풀메뉴 닫기');
            }
          }); */
          ...
          /*var clickMenu = $(".gnb-category__menu li a").filter(function(a,b){
            return $(location).attr("pathname").indexOf($(b).attr("href")) > -1
                
          });
          
          if(clickMenu.length > 0){
            clickMenu.parent().siblings().removeClass('is-active');
            clickMenu.parent().addClass('is-active');
          }*/
          
          // 추천 검색어 셋팅
          recommSrchw();
          
           $("#searchKwdBtn").on('click', function(e){
            e.preventDefault();
            if("Y"=='Y'){
              $.eshopfront.search.searchCommunityKwd();//searchCommunityKwd
            }else{
              $.eshopfront.search.searchKwd();
            }
          });
           
          $("#searchKeyword").on("keyup", searchKeyUpEvent);
          $("#searchKeyword").on("click", searchClickEvent);
          
          //최근 검색어 단건 삭제
          $(document).on("click",".button-search-content__del",function(e){
            e.stopPropagation();
            var data = "";
            data = {"txt" : encodeURIComponent($(this).parent().find("a").text())}
            if("Y" != 'Y'){
              $.eshopfront.cmm.deleteHistory(data);
            }else{
              $.eshopfront.cmm.deleteCommunityHistory(data);
            }
            });
          
          //최근 검색어 전체 삭제
          $(".button-search-all__del").on("click", function() {		
                var data = [];
               $.each($(".lately-complete ul li").find("a"), function (index, item){
                   data.push(encodeURIComponent($(item).text()));
               });
               data = {"txt":  data.join(",")};
               $.eshopfront.cmm.deleteHistory(data);
           });
          ...
          //인기 검색어
          $("#tab-search2").on("click", function() {
            if("Y" != "Y"){
              $.eshopfront.cmm.popularSrchw();
            }else{
              $.eshopfront.cmm.popularCommunitySrchw();
            }
          });
          
          //검색어창 사이트 셀렉트 박스
          /* var $dr = $(".header-search__dropdown .dropdown-item").filter(function(){ return $(this).data("url") == 'www.hyundailivart.co.kr'})
          $dr.addClass('is-active').siblings().removeClass('is-active');	
          $dr.closest('.dropdown').find('[aria-controls="dropdown-menu"]').children('span').eq(0).text($dr.text()); */
        ...
        function searchKeyUpEvent(){
          idx = $.eshopfront.search.searchKeyUpEvent(event, idx);
        }
        ...
        function searchClickEvent(){
          ...
          if(kwd== ""){
            if("Y" != 'Y'){
              $.eshopfront.search.srchwHistory();
            }else{
              $.eshopfront.search.srchwCommunityHistory();
            }
          }else{
              $(".auto-complete ul.result-compl").html("");
            $(".result-promo").remove();
        
            if(!$(".auto-complete").hasClass("hide")){
               $(".auto-complete").addClass("hide");
            }else{
              $.eshopfront.cmm.kwdAutoComplete(kwd);
            }
          }
        }
  ...
  $(document).ready(function() {
      ...
      $(document).ready(function(){
        ...
        /*히스토리 탭 이동
        var tab = $.sessionStorage.get("tab");
        if(tab != null && tab != ''){
          $(tab+"_id").addClass('is-active').siblings().removeClass('is-active');
              $("#magazineD000000489 .d-tab").hide();
          $(tab).show();
        }*/
  
        var tab = "#D000000489tab1";
      $(tab).show();
      
         // 제목 탭 블록
          $(document).on('click', '#magazineD000000489 .tab-block-item', function (e) {
              e.preventDefault();
              $(this).addClass('is-active').siblings().removeClass('is-active');
              $("#magazineD000000489 .d-tab").hide();
              $($(this).data("sn")).show();
              //$.sessionStorage.set("tab", $(this).data("sn"));
          });       
  ...
  function showMasonry() {
    if(typeof Masonry == "function"){
      $('.masonry-grid-item').removeClass('loading');
      $("#open-houseD000000442 .loading-container").removeClass("is-active");
      ...
      var totalCnt = Number($("#ophusCnt").val()); //이미지 호출 갯수
      var tmpCnt = $("#ophusCnt").data("loaded") ? Number($("#ophusCnt").data("loaded")) : totalCnt;	//이미지 호출완료 횟수
      //console.log(totalCnt+"::::"+tmpCnt)
      if(totalCnt <= tmpCnt){
        //전체 호출 완료 -> 인터벌 제거
        if(ophusIntv) {
          clearInterval(ophusIntv);
          //console.log("clear")
        }
      }
    }
  }
  function onLoadMasonry(){
    if(ophusIntv){
      clearInterval(ophusIntv);
    }
    //0.5초후 재정렬 시도
    ophusIntv = setInterval(showMasonry, 500);
  ...
  $(document).ready(function () {
    ...
    $(document).on("click","#open-houseD000000442  .icon-category-lists li,#open-houseD000000442 .hash-tag-lists a,#open-houseD000000442 .button-mainmodule-more",
            function(){
      searchCommunity(this, true);
    });
  
    // sessionStorage 검사 조건이 유무 검사
    if($.sessionStorage.get($.eshopfront.MNOPHUS_FILTER)){
      searchBySavedCondition($.sessionStorage.get($.eshopfront.MNOPHUS_FILTER));
    } else {
      searchCommunity(this, false);
    }
    
    $(document).on("click", ".a-codtl",function(e){
  
      var data = {pageNo : 1, type : "all", sortBy : "rgstdtime", cmpoSn : "D000000442"};
      //필터 선택
      if($("#open-houseD000000442 .icon-category-lists li.is-active").length > 0){
        var filter4 =  $($("#open-houseD000000442 .icon-category-lists li.is-active")[0]).data("filter4");
        var sortBy = $($("#open-houseD000000442 .icon-category-lists li.is-active")[0]).data("sort");
        if(filter4!==""){
      ...
      if($("#open-houseD000000442 .hash-tag-lists a.is-active").length > 0){
        data.type= $($("#open-houseD000000442 .hash-tag-lists a.is-active")[0]).data("typecd");
      }
      $.sessionStorage.set($.eshopfront.MNOPHUS_FILTER, data);
      // 페이지 이동
      $(location).attr("href",$(this).data("url"));
      historySetting($(document).scrollTop());
    })
  });
  
  // 커뮤니티 검색 isNotFirst 처음 로딩할때 false
  function searchCommunity(obj, isNotFirst){
    var data = {pageNo : 1, type : "all", sortBy : "rgstdtime", cmpoSn : "D000000442"};
    if(!isNotFirst){
      $("#open-houseD000000442 [data-sort='cncnCnt1m']").addClass('is-active');
      $("#open-houseD000000442 [data-filter4=''][data-sort='cncnCnt1m'] a").addClass('is-active');
    }
    //필터
    if($(obj).data("filter4") !== undefined){
      $(obj).addClass('is-active').siblings().removeClass('is-active');
      // ALL or BEST 클릭시 #해시태그 모두 비활성화
      $("#open-houseD000000442 .hash-tag-lists a").removeClass('is-active');
      data.pageNo=1;
    }
    //정렬
    if($(obj).data("typecd") !== undefined){
      $(obj).addClass('is-active').siblings().removeClass('is-active');
      data.pageNo=1;
    }
    //더보기
    if($(obj).data("addmore2") == true){
      var pageNo = Number($("#open-houseD000000442 [name=pageNo]").val());
    ...
    //페이지 셋팅
    $("#open-houseD000000442 [name=pageNo]").val(data.pageNo);
    $.sessionStorage.remove($.eshopfront.MNOPHUS_FILTER);
    var $pos = $(document).scrollTop();
    $.ajax({
      url: '/module/openHouseList/'+data.type,
      type : "post",
      dataType : "html",
      data : data,
      headers : {
      ...
      beforeSend: function(){
        $("#open-houseD000000442 .loading-container").addClass("is-active");
        $("#open-houseD000000442 .button-mainmodule-more").hide();
      },
      success : function(html){
        if(data.pageNo == 1 && isNotFirst){ // 더보기 버튼 클릭 시
          $grid.masonry('remove', $('.masonry-grid-item'));
        }
        if(isNotFirst){
          $content = $(html);
        ...
        setTimeout(function(){
          if($("#open-houseD000000442 input[name='totalCnt']").length ==0 || Number(data.pageNo) * 8 >= Number($("#open-houseD000000442 input[name='totalCnt']").val())){
            $("#open-houseD000000442 .button-mainmodule-more").hide();
          }else{
            $("#open-houseD000000442 .button-mainmodule-more").show();
          }
        },500);
        //이미지 로드갯수 초기화
        onLoadCnt = 0;
  ...
  // 저장된 조건으로 검색
  function searchBySavedCondition(data){
    var state = history.state;
    //console.log(history);
    data.pageNo = state != null ? state.page : 1;
  
    if(data.sortBy !== "cncnCnt1m"){
      $("#open-houseD000000442 [data-sort='"+data.sortBy+"']").addClass('is-active').siblings().removeClass('is-active');
      $("#open-houseD000000442 [data-sort='"+data.sortBy+"'] a").addClass('is-active').siblings().removeClass('is-active');
    } else {
      $("#open-houseD000000442 [data-filter4='"+data.filter4+"']").addClass('is-active').siblings().removeClass('is-active');
      $("#open-houseD000000442 [data-filter4='"+data.filter4+"'] a").addClass('is-active').siblings().removeClass('is-active');
    }
  
    if(data.type !== "all"){
      $("#open-houseD000000442 [data-typecd='"+data.type+"']").addClass('is-active').siblings().removeClass('is-active');
    } else {
      $("#open-houseD000000442 .hash-tag-lists a").removeClass('is-active');
    }
    if(state === null){
      ...
      setTimeout(function(){
        if(history && history.scrollRestoration === "manual"){
          ...
          if(state && state.scroll) {
            ...
            $('html, body').animate({ scrollTop: state.scroll + 1 }, 10);
  ...
  function historySetting(arg){
    //console.log(arg)
    if(arg === 0){
      history.pushState({list:$(".masonry-grid.grid-item-lists").html(),page:$("#open-houseD000000442 [name=pageNo]").val()},'','/community');
    }else{
      history.pushState({list:$(".masonry-grid.grid-item-lists").html(),page:$("#open-houseD000000442 [name=pageNo]").val(),scroll:arg},'','/community');
  ...
  // 더보기
  function getMasterHouseList(){
    // pageNo 증가
    $("#mnmoduleD000000475 [name='pageNo']").val(Number($("#mnmoduleD000000475 [name='pageNo']").val()) + 1);	
    
    var data = {
      pageNo : $("#mnmoduleD000000475 [name='pageNo']").val(),
      sortBy : "cncnCnt",
      cmpoSn : "D000000475"
    }
    
    $.ajax({
      url: '/module/masterHouse/export',
      type : "post",
      dataType : "html",
      data : data,
      headers : {
      ...
      success : function(html){
        $("#mnmoduleD000000475 .grid-item-lists").append(html);	
        
        if(Number(data.pageNo) * 8 >= Number($("#mnmoduleD000000475 [name='totalCnt']").val())){
          $("#mnmoduleD000000475 .button-more button").hide();
        }
  ...
  $(document).ready(function() {
    //shuffleNode("#d-keyvisualD100000000", ".swiper-slide");
      ...
      loadSwiperWideBanner04('#main-keyvisualD100000000');
  });
  
  function shuffleNode(parent, children){
    $(parent).html($(parent+" "+children).sort(function(a,b){return 0.5 - Math.random()}));
  }
    ...
    $(document).ready(function(){
      if( $.eshopfront.cmm.isEmpty( $.eshopfront.cmm.getCookie("mbrSn") ) ){
        ...
        $.eshopfront.cmm.setCookie("unMbrNo", '', 1,".hyundailivart.co.kr");
        $.eshopfront.cmm.setCookie("mbrSn", '', 1,".hyundailivart.co.kr");
      }
      // 유닛용 툴팁 선언을 위한 스크립트 : 200701
  ...
  if((location.href.indexOf("/index")) > 0){
    ...
    window.criteo_q.push(
     { event: "setAccount", account: 19435},
  ...
  (function() {
    return {
        wp_hcuid:wp_id,   /*고객넘버 등 Unique ID (ex. 로그인  ID, 고객넘버 등 )를 암호화하여 대입.
    ...
    $(document).ready(function(){
      ...
      if($.eshopfront.cmm.getCookie("CO_IS_HUMAN") =="Y"){
        $.eshopfront.cmm.removeCookie("CO_IS_HUMAN",".hyundailivart.co.kr");
        $("#modal_drmcMember").addClass("is-active");	
      }
      //휴면 해제 버튼
      $(".restCancelBtn").on("click",function(){
        $('#modal_drmcMember').removeClass('is-active');
      });
    });
  </script>
  ...
  $(document).on("click", ".modal-layer-pop .button-close-popup", function(){
    $.eshopfront.cmm.setCookie("popup_"+$(this).data("pop"), "DO_NOT_SHOW_1_DAY", 1, window.location.host);
    $("#popup_"+$(this).data("pop")).remove();
  });
  
  $(document).ready(function() {
    $.ajax({
      url : "/popup/popupListAjax",
      type : "GET",
      dataType : "html",
      headers: {
        'X-CSRF-TOKEN': $('meta[name="X-CSRF-TOKEN"]').attr('content')
